...
function Stripe() {
    ...
    useEffect(() => {
      ...
      if (cart.quantity >= 10 && cart.quantity < 20 ) {
          cost = cart.price * cart.quantity * .95
      }
      else if (cart.quantity > 20) {
          cost = cart.price * cart.quantity * .9
      }
      else {
...
//4242 4242 4242 4242
  ...
  return (
    <div className='checkout-container'>
        { !loading ? (
        <>
        <p> Would you like to purchase {cart.quantity} lbs of { cart.product ? <p>{cart.product.name}?</p> : null } for ${cost}</p>
